...
export const CartItem = ({ item }) => {
  ...
  return (
    <Fragment key={item.product.sku}>
      <div className=""><Image width={100} height={100} src={`https://picsum.photos/id/${item.product.id}/100`}></Image></div>
      <div className="flex flex-col">{item.product.name}
        <span className="text-sm">{item.product.sku}</span>
        <div><span className="text-xs text-red-400 cursor-pointer" onClick={handleRemoveClick}>Remover</span></div>
      </div>
      <div className="text-lg flex-grow-0">
      ...
      <div>R${item.product.price.toFixed(2)}</div>
      <div>R${(item.product.price * qty).toFixed(2)}</div>
    </Fragment>
  )
};
